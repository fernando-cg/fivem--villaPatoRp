...
AddEventHandler('esx_jobChat:311', function(targetCoords, msg, streetName, emergency)
	...
	if emergency == '311' then
		messageFull = {
			template = '<div style="padding: 8px; margin: 8px; background-color: rgba(255, 51, 51); border-radius: 25px;"><i class="fas fa-bell"style="font-size:15px"></i> [CR]: {0} | Location : {1} | {2}</font></i></b></div>',
...
AddEventHandler('esx_jobChat:911', function(targetCoords, msg, streetName, emergency)
	...
	if emergency == '911' then
		messageFull = {
			template = '<div style="padding: 8px; margin: 8px; background-color: rgba(0, 38, 153); border-radius: 25px;"><i class="fas fa-bell"style="font-size:15px"></i> [CNP] : {0} | Location : {1} | {2}</font></i></b></div>',
...
RegisterServerEvent('esx_jobChat:forzar')
AddEventHandler('esx_jobChat:forzar', function(targetCoords, msg, streetName, emergency)
    local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	fal = xPlayer.getName(source)
	local messageFull
	if emergency == 'forzar' then
		messageFull = {
        	...
        	args = {"Robo de coche en", streetName,""}
		}
	end
	TriggerClientEvent('esx_jobChat:ForzarMarker', -1, targetCoords, '911')
	TriggerClientEvent('esx_jobChat:911EmergencySend', -1, messageFull)
end)
...
AddEventHandler('esx_jobChat:mech', function(targetCoords, msg, streetName, emergency)
	...
	if emergency == 'mech' then
		messageFull = {
			template = '<div style="padding: 8px; margin: 8px; background-color: rgba(128, 64, 0); border-radius: 25px;"><i class="fas fa-bell"style="font-size:15px"></i> [Mec] : {0} | Location : {1} | {2}</font></i></b></div>',
        	args = {fal, streetName, msg}
		}
	end
