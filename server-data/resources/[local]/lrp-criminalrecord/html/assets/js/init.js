$(document).ready(function() {
  ...
  $('.back, .logo').click(function() {
    ...
    if ( subPage == null ) {
      ...
      $.post('http://lrp-criminalrecord/fetch', JSON.stringify({
        type: 'start'
      }), function( cb ) {
  ...
  $('#submit-search').click(function() {
    ...
    if ( offense.length > 0 ) {
      ...
      $.post('http://lrp-criminalrecord/search', JSON.stringify( {
  ...
  $('#submit-add').click(function() {
    ...
    if ( firstname.length > 0 && lastname.length > 0 && dob.length > 0 && charge.length > 0 && description.length > 0 && term.length > 0 && date.length > 0 ) {
      $.post('http://lrp-criminalrecord/add', JSON.stringify({
  ...
  $('#submit-update').click(function() {
    $.post('http://lrp-criminalrecord/update', JSON.stringify({
  ...
  $('#submit-delete').click(function() {
    $.post('http://lrp-criminalrecord/remove', JSON.stringify({
      offense: $('#signature .r-offense').text()
    }), function( cb ) {
      $('#welcome tbody').html('');
  ...
  $(document).keyup(function(e) {
    if (e.keyCode == 27) {
      reset();
      $.post('http://lrp-criminalrecord/escape', JSON.stringify({}));
    }
  });
