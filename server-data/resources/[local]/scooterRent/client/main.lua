		...
		Citizen.Wait(0)
			...
			local coords    = GetEntityCoords(playerPed)
				if(GetDistanceBetweenCoords(coords, vehiclesNeedsToCreate[i].x, vehiclesNeedsToCreate[i].y, vehiclesNeedsToCreate[i].z, true) < Config.DistanseSpawn) then
					...
					DrawText3Ds(vehiclesNeedsToCreate[i].x ,vehiclesNeedsToCreate[i].y ,vehiclesNeedsToCreate[i].z, "Pulsa F para abrir el menu" )
					end
...
ESX.ShowNotification("~g~Alquiler completo")
end)
...
table.insert(elements, {
      name    = 'rentTime',
      label   = 'Ajusta el tiempo (Minutos)',
      value   = 0,
      type    = 'slider',
      max     = 360
    })
...
ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclesRentrent_this_veh',
	...
	function(data, menu)
        ...
        { 
          ...
          align = 'top-left',
          elements = {
            {label = '<span style="color:Yellow;">Rent by</span> <span style="color:Green;">' .. curRentPrice  .. '$ ?</span>', value = 'lol'},
        ...
        function (data2, menu2)
			...
			ESX.TriggerServerCallback('esx_vehiclesRent:buyVehicle', function(isBuyed)
				...
				ESX.ShowNotification("~g~Has alquilado un patinete!")
				else 
	  ...
	  elseif not price then 
        ...
        function (data2, menu2)
	  	...
	  	ESX.ShowNotification("~r~No puedes alquilar por 0 minutos.")
	  end
	  end
	...
	while true do
		Citizen.Wait(0)
			...
			for i = 1, #spawnedVehs, 1 do
				...
				AddTextComponentString('Pulsa ~INPUT_CONTEXT~ Para entrar en el menu')
				...
				founded = true
				SetTextComponentFormat('STRING')
				AddTextComponentString('Pulsa ~INPUT_CONTEXT~ Para coger un patinete')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0, 38) then
				if rentJob >= Config.MaxJob then 
					ESX.ShowNotification("No puedes alquilar ~y~" .. Config.MaxJob .. " Patinetes!")
					else 
					OpenVehicleMenu(spawnedVehs[i])
				end
				end
				...
				founded = true
				SetTextComponentFormat('STRING')
				AddTextComponentString('You cant take this car!')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				end
				end
				end
